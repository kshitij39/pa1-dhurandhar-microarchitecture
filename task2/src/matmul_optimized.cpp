                ...
                for (int j = j_blk; j + 1 < j_end; j += 2) {
                    ...
                    for (int k_blk = 0; k_blk < K; k_blk += BLOCK_K) {
                        ...
                        for (; k + 7 < k_end; k += 8) {
                             ...
                             *
                             * degree = 1:
                             *   prefetch one future location
                             *
                             * degree = 2:
                             *   prefetch two future locations
                             *
                             * etc.
                             */
                            for (int d = 1; d <= PREFETCH_DEGREE; d++) {
                                ...
                                int pf_k =
                                    k + PREFETCH_DISTANCE * d;

                                if (pf_k < k_end) {

                                    _mm_prefetch(
                                        (const char*)&A[i * lda + pf_k],
                                        _MM_HINT_T0
                                    );

                                    _mm_prefetch(
                                        (const char*)&A[(i + 1) * lda + pf_k],
                                        _MM_HINT_T0
                                    );

                                    _mm_prefetch(
                                        (const char*)&A[(i + 2) * lda + pf_k],
                                        _MM_HINT_T0
                                    );

                                    _mm_prefetch(
                                        (const char*)&A[(i + 3) * lda + pf_k],
                                        _MM_HINT_T0
                    
                                    );

                                    _mm_prefetch(
                                        (const char*)&B[j * ldb + pf_k],
                                        _MM_HINT_T0
                                    );

                                    _mm_prefetch(
                                        (const char*)&B[(j + 1) * ldb + pf_k],
                                        _MM_HINT_T0
                                    );
                                }
                            }
                    ...
                    C[i * ldc + j] =
                        reduce_sum(sum00) + tail[0][0];

                    C[i * ldc + j + 1] =
                        reduce_sum(sum01) + tail[0][1];

                    C[(i + 1) * ldc + j] =
                        reduce_sum(sum10) + tail[1][0];

                    C[(i + 1) * ldc + j + 1] =
                        reduce_sum(sum11) + tail[1][1];

                    C[(i + 2) * ldc + j] =
                        reduce_sum(sum20) + tail[2][0];

                    C[(i + 2) * ldc + j + 1] =
                        reduce_sum(sum21) + tail[2][1];

                    C[(i + 3) * ldc + j] =
                        reduce_sum(sum30) + tail[3][0];

                    C[(i + 3) * ldc + j + 1] =
                        reduce_sum(sum31) + tail[3][1];
                }
